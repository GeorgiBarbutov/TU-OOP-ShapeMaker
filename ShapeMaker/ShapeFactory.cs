...
namespace ShapeMaker
{
    public class ShapeFactory : IShapeFactory
    {
        public IShape CreateCicle(float centerXCoordinate, 
            float centerYCoordinate, float radius, Color color)
        {
            IShape circle = new Circle(
                new PointF(centerXCoordinate, centerYCoordinate),  radius, color);

            return circle;
        }

        public IShape CreateRectangle(float pointXCoordinate, 
            float pointYCoordinate, float width, float height, Color color)
        {
